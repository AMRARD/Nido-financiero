...
const STORAGE_KEY = "nido_financiero_app";
...
const LOGIN_VACIO = { username: "", password: "" };
const REGISTRO_VACIO = { nombre: "", username: "", password: "", confirmarPassword: "" };
  ...
  try {
    ...
    return {
      ...
      users: estado.users.map((user) => ({
        ...user,
        data: {
          ingresos: (user.data?.ingresos || []).map((item) => ({ ...item, fecha: normalizarFecha(item.fecha) })),
          gastos: (user.data?.gastos || []).map((item) => ({ ...item, fecha: normalizarFecha(item.fecha) })),
...
function Campo(props) {
  return <input className="rounded-2xl border border-slate-300 px-4 py-3 outline-none focus:border-slate-500" {...props} />;
}

function Select(props) {
  return <select className="rounded-2xl border border-slate-300 px-4 py-3 outline-none focus:border-slate-500" {...props} />;
}
    ...
    <div className="min-h-screen bg-slate-100 p-4 md:p-8">
      <div className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-2">
        <div className="rounded-3xl bg-slate-900 p-8 text-white shadow-sm">
          ...
          <p className="mt-4 text-slate-300">Crea usuarios para que cada persona tenga acceso a sus cuentas, deudas, gráficos y reportes.</p>
          <div className="mt-8 rounded-3xl border border-slate-700 bg-slate-800 p-5">
            ...
            <p className="mt-2 font-semibold">Usuario: admin</p>
        ...
        <div className="space-y-6">
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-4 grid gap-3">
              <Campo placeholder="Usuario" value={loginForm.username} onChange={(e) => setLoginForm((p) => ({ ...p, username: e.target.value }))} />
              <Campo type="password" placeholder="Contraseña" value={loginForm.password} onChange={(e) => setLoginForm((p) => ({ ...p, password: e.target.value }))} />
              <button onClick={onLogin} className="rounded-2xl bg-slate-900 px-5 py-3 font-medium text-white hover:opacity-90">Entrar</button>
            </div>
          </div>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <Campo placeholder="Nombre" value={registerForm.nombre} onChange={(e) => setRegisterForm((p) => ({ ...p, nombre: e.target.value }))} />
              <Campo placeholder="Usuario" value={registerForm.username} onChange={(e) => setRegisterForm((p) => ({ ...p, username: e.target.value }))} />
              <Campo type="password" placeholder="Contraseña" value={registerForm.password} onChange={(e) => setRegisterForm((p) => ({ ...p, password: e.target.value }))} />
              <Campo type="password" placeholder="Confirmar contraseña" value={registerForm.confirmarPassword} onChange={(e) => setRegisterForm((p) => ({ ...p, confirmarPassword: e.target.value }))} />
            </div>
            <button onClick={onRegister} className="mt-4 rounded-2xl bg-slate-900 px-5 py-3 font-medium text-white hover:opacity-90">Crear usuario</button>
          </div>

          {mensajeAuth ? <div className="rounded-2xl bg-white px-4 py-3 text-sm text-slate-700 shadow-sm">{mensajeAuth}</div> : null}
        </div>
      </div>
    </div>
  );
...
export default function PresupuestoWeb() {
  ...
  const currentUser = useMemo(() => appState.users.find((user) => user.id === appState.currentUserId) || null, [appState]);
  ...
  const actualizarDatosUsuario = (updater) => {
    ...
    setAppState((prev) => ({
      ...prev,
      users: prev.users.map((user) => (user.id !== prev.currentUserId ? user : { ...user, data: updater(user.data) })),
    }));
  };
  ...
  const iniciarSesion = () => {
    ...
    if (!username || !password) {
      mostrarMensajeAuth("Completa usuario y contraseña.");
      return;
    }

    const user = appState.users.find((item) => item.username.toLowerCase() === username.toLowerCase() && item.password === password);
    if (!user) {
  ...
  const crearUsuario = () => {
    ...
    if (!nombre || !username || !password || !confirmarPassword) {
    ...
    if (appState.users.some((item) => item.username.toLowerCase() === username.toLowerCase())) {
      mostrarMensajeAuth("Ese nombre de usuario ya existe.");
      return;
    }
    ...
    const nuevoUsuario = {
      ...
      password,
      role: "user",
      data: { ingresos: [], gastos: [], cuentas: [], deudas: [] },
    };
  ...
  const agregarGasto = () => {
    ...
    actualizarDatosUsuario((data) => ({
      ...data,
      gastos: [...data.gastos, { id: crearId(), descripcion, monto, categoria, cuentaId: cuentaId || null, fecha: normalizarFecha(nuevoGasto.fecha) }],
      cuentas: cuentaId ? data.cuentas.map((item) => (item.id === cuentaId ? { ...item, saldo: item.saldo - monto } : item)) : data.cuentas,
    }));
    setNuevoGasto(GASTO_VACIO);
  ...
  const agregarCuenta = () => {
    ...
    actualizarDatosUsuario((data) => ({ ...data, cuentas: [...data.cuentas, { id: crearId(), nombre, tipo, saldo }] }));
    setNuevaCuenta(CUENTA_VACIA);
  ...
  const agregarDeuda = () => {
    ...
    actualizarDatosUsuario((data) => ({ ...data, deudas: [...data.deudas, { id: crearId(), nombre, saldo }] }));
    setNuevaDeuda(DEUDA_VACIA);
  ...
  const pagarDeudaDesdeCuenta = () => {
    ...
    actualizarDatosUsuario((data) => ({
      ...
      deudas: data.deudas.map((item) => (item.id === deudaId ? { ...item, saldo: Math.max(0, item.saldo - monto) } : item)),
    }));
    setPagoDeuda(PAGO_DEUDA_VACIO);
    mostrarMensaje("Pago aplicado correctamente a la deuda.");
  };

  const eliminarIngreso = (id) => actualizarDatosUsuario((data) => ({ ...data, ingresos: data.ingresos.filter((item) => item.id !== id) }));
  const eliminarGasto = (id) => actualizarDatosUsuario((data) => ({ ...data, gastos: data.gastos.filter((item) => item.id !== id) }));
  const eliminarCuenta = (id) => actualizarDatosUsuario((data) => ({ ...data, cuentas: data.cuentas.filter((item) => item.id !== id) }));
  const eliminarDeuda = (id) => actualizarDatosUsuario((data) => ({ ...data, deudas: data.deudas.filter((item) => item.id !== id) }));

  if (!currentUser) {
    return <AuthPanel onLogin={iniciarSesion} onRegister={crearUsuario} loginForm={loginForm} setLoginForm={setLoginForm} registerForm={registerForm} setRegisterForm={setRegisterForm} mensajeAuth={mensajeAuth} />;
  }
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-2 text-slate-600">Registra ingresos, gastos, saldos por cuenta, deudas y movimientos entre tus fondos.</p>
              <p className="mt-2 text-sm text-slate-500">Sesión activa: <span className="font-semibold text-slate-900">{currentUser.nombre}</span> (@{currentUser.username})</p>
            </div>
            <button onClick={cerrarSesion} className="rounded-2xl border border-slate-300 px-5 py-3 font-medium text-slate-700 hover:bg-slate-100">Cerrar sesión</button>
          </div>
          {mensaje ? <div className="mt-4 rounded-2xl bg-slate-100 px-4 py-3 text-sm text-slate-700">{mensaje}</div> : null}
        </div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <Select value={tipoReporte} onChange={(e) => setTipoReporte(e.target.value)}>
                {TIPOS_REPORTE.map((tipo) => <option key={tipo.value} value={tipo.value}>{tipo.label}</option>)}
              </Select>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-4">
            ...
            <TarjetaResumen titulo="Movimientos" value={undefined} valor={String(ingresosReporte.length + gastosReporte.length)} />
          </div>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={categoriasReporte} dataKey="monto" nameKey="categoria" cx="50%" cy="50%" outerRadius={95} label>
                      {categoriasReporte.map((entry, index) => <Cell key={entry.categoria} fill={COLORES[index % COLORES.length]} />)}
                    </Pie>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={datosCategorias} dataKey="monto" nameKey="categoria" cx="50%" cy="50%" outerRadius={110} label>
                    {datosCategorias.map((entry, index) => <Cell key={entry.categoria} fill={COLORES[index % COLORES.length]} />)}
                  </Pie>
                  <Tooltip formatter={(value) => formatoMoneda(value)} />
                  <Legend />
                </PieChart>
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          <div className="space-y-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <Campo placeholder="Descripción" value={nuevoIngreso.descripcion} onChange={(e) => setNuevoIngreso((p) => ({ ...p, descripcion: e.target.value }))} />
              <Campo type="number" min="0" step="0.01" placeholder="Monto" value={nuevoIngreso.monto} onChange={(e) => setNuevoIngreso((p) => ({ ...p, monto: e.target.value }))} />
              <Campo type="date" value={nuevoIngreso.fecha} onChange={(e) => setNuevoIngreso((p) => ({ ...p, fecha: e.target.value || HOY }))} />
            </div>
          ...
          <div className="space-y-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <Campo placeholder="Descripción" value={nuevoGasto.descripcion} onChange={(e) => setNuevoGasto((p) => ({ ...p, descripcion: e.target.value }))} />
              <Campo placeholder="Categoría" value={nuevoGasto.categoria} onChange={(e) => setNuevoGasto((p) => ({ ...p, categoria: e.target.value }))} />
              <Campo type="number" min="0" step="0.01" placeholder="Monto" value={nuevoGasto.monto} onChange={(e) => setNuevoGasto((p) => ({ ...p, monto: e.target.value }))} />
              <Campo type="date" value={nuevoGasto.fecha} onChange={(e) => setNuevoGasto((p) => ({ ...p, fecha: e.target.value || HOY }))} />
              <Select value={nuevoGasto.cuentaId} onChange={(e) => setNuevoGasto((p) => ({ ...p, cuentaId: e.target.value }))}>
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          <div className="space-y-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="grid gap-3 md:grid-cols-3">
              <Campo placeholder="Nombre" value={nuevaCuenta.nombre} onChange={(e) => setNuevaCuenta((p) => ({ ...p, nombre: e.target.value }))} />
              <Campo placeholder="Tipo" value={nuevaCuenta.tipo} onChange={(e) => setNuevaCuenta((p) => ({ ...p, tipo: e.target.value }))} />
              <Campo type="number" min="0" step="0.01" placeholder="Saldo inicial" value={nuevaCuenta.saldo} onChange={(e) => setNuevaCuenta((p) => ({ ...p, saldo: e.target.value }))} />
            </div>
          ...
          <div className="space-y-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <Campo placeholder="Nombre de la deuda" value={nuevaDeuda.nombre} onChange={(e) => setNuevaDeuda((p) => ({ ...p, nombre: e.target.value }))} />
              <Campo type="number" min="0" step="0.01" placeholder="Saldo pendiente" value={nuevaDeuda.saldo} onChange={(e) => setNuevaDeuda((p) => ({ ...p, saldo: e.target.value }))} />
            </div>
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          <div className="space-y-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="grid gap-3 md:grid-cols-3">
              <Select value={movimiento.origenId} onChange={(e) => setMovimiento((p) => ({ ...p, origenId: e.target.value }))}>
                <option value="">Cuenta origen</option>
                {cuentas.map((cuenta) => <option key={cuenta.id} value={cuenta.id}>{cuenta.nombre}</option>)}
              </Select>
              <Select value={movimiento.destinoId} onChange={(e) => setMovimiento((p) => ({ ...p, destinoId: e.target.value }))}>
                <option value="">Cuenta destino</option>
                {cuentas.map((cuenta) => <option key={cuenta.id} value={cuenta.id}>{cuenta.nombre}</option>)}
              </Select>
              <Campo type="number" min="0" step="0.01" placeholder="Monto" value={movimiento.monto} onChange={(e) => setMovimiento((p) => ({ ...p, monto: e.target.value }))} />
            </div>
          ...
          <div className="space-y-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="grid gap-3 md:grid-cols-3">
              <Select value={pagoDeuda.cuentaId} onChange={(e) => setPagoDeuda((p) => ({ ...p, cuentaId: e.target.value }))}>
                <option value="">Cuenta</option>
                {cuentas.map((cuenta) => <option key={cuenta.id} value={cuenta.id}>{cuenta.nombre}</option>)}
              </Select>
              <Select value={pagoDeuda.deudaId} onChange={(e) => setPagoDeuda((p) => ({ ...p, deudaId: e.target.value }))}>
                <option value="">Deuda</option>
                {deudas.map((deuda) => <option key={deuda.id} value={deuda.id}>{deuda.nombre}</option>)}
              </Select>
              <Campo type="number" min="0" step="0.01" placeholder="Monto" value={pagoDeuda.monto} onChange={(e) => setPagoDeuda((p) => ({ ...p, monto: e.target.value }))} />
            </div>
